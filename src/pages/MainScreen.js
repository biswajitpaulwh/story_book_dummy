...
export default class MainScreen extends Component {
  ...
  renderItem = ({item}) => {
    return (
      <TouchableOpacity style={{flex:1, flexDirection:'row', marginBottom:3, margin: 12}}>
        <Image style={{width:80, height:80,}} source={{ uri: item.thumbnailUrl }}></Image>
        <View style={{flex:1, justifyContent:'center'}}>
          <Text style={{paddingLeft: 20, color:'#FFFFFF',fontWeight: "bold", fontSize:16, paddingBottom:10}}>{item.title}</Text>
          <Text style={{paddingLeft: 20, color:'#FFFFFF', fontSize:12}}>{item.authors}</Text>
        </View>
